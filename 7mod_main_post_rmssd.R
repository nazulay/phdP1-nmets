# not sure all results are updated to post rmssd
...
hrv <- long[!is.na(long$PT_POST_CPT_RMSSD_T6),] 
...
fit_cat_nmets <- with(hrv, lm(PT_POST_CPT_RMSSD_T6~SEX_T6+AGE_T6+n_mets6_cat))
...
# rep() for them to have the same dimentions

qplot(x=term, y=estimate, data=nmets_ci) + geom_errorbar(aes(ymin=lower, ymax=upper, width=0.15)) +
  labs(y="rmssd post CPT", x="Number of MetS factors or diabetes")

table(hrv_na$n_mets6_cat, useNA = "ifany")
table(hrv_na$n_mets[hrv_na$n_mets6 == 6])

write.xlsx(nmets_ci, file='plotdata_gp_post_rmssd.xlsx', sheetName = 'nmets_ci')
...
fit.mitml <- with(implist1, lm(PT_POST_CPT_RMSSD_T6~SEX_T6+AGE_T6+n_mets6_cat))
...
#write.xlsx(tuk, file='plotdata_gp_post_rmssd.xlsx', sheetName = 'tukey', append=TRUE)
...
f <- lm(PT_POST_CPT_RMSSD_T6~SEX_T6+AGE_T6+n_mets6_cat, data=hrv_na)
summary(f)  # same as imputed (almost on the decimal)
Anova(f)
anova(f) # why different numbers?
Anova(f, lm(PT_POST_CPT_RMSSD_T6~SEX_T6+AGE_T6+n_mets6_cat, data=hrv_na), type=3)
aov(f)
...
# ---- SPECIFIC FACTORS ----
# ---- singles ----
fit_singles <- with(hrv, lm(PT_POST_CPT_RMSSD_T6~SEX_T6+AGE_T6+central_obesity+high_triglyc+
                              low_hdl+high_BP+high_glucose + diabetic))

summary(pool(fit_singles))  # all significant

pool.r.squared(fit_singles, adjusted = TRUE)  # 0.05625244   
...
write.xlsx(metsfac, file='plotdata_gp_post_rmssd.xlsx', sheetName = 'mets_factors', append=TRUE)
...
# ---- model assumptions ----
# -- no imp
noimp_singles <- lm(PT_POST_CPT_RMSSD_T6~SEX_T6+AGE_T6+central_obesity+high_triglyc+
...
# ---- pairs ----
fit_pairs <- with(hrv, lm(PT_POST_CPT_RMSSD_T6~central_obesity*high_triglyc+
...
pool.r.squared(fit_pairs, adjusted = TRUE)  # 0.05551422    
...
# ---- unimputed ----
noimp_pairs <- lm(PT_POST_CPT_RMSSD_T6~central_obesity*high_triglyc+
                    central_obesity*low_hdl + central_obesity*high_BP +
                    central_obesity*high_glucose + high_triglyc*low_hdl + 
                    high_triglyc*high_BP + high_triglyc*high_glucose +
                    low_hdl*high_BP + low_hdl*high_glucose + high_BP*high_glucose +
...
# ---- triplets ----
fit_triplets <- with(hrv, lm(PT_POST_CPT_RMSSD_T6~
