...
hrv <- long[!is.na(long$PT_PRE_CPT_SDNN_T6),] 
hrv_na <- hrv[hrv$.imp==0,]
hrv <- as.mids(hrv)  # warning because attended_cpt is now a constant, out

hrv.women <- filter(hrv, SEX_T6==0)
hrv.men <- filter(hrv, SEX_T6==1)

#how many diabetics, MetS in imputed datasets
a <- c()
b <- c()
for (i in 1:30) {
  a <- c(a, sum(complete(hrv, i)$diabetic))
  b <- c(b, sum(complete(hrv, i)$n_mets6 %in% c(3, 4, 5)))
}
range(a)  # 497-504
range(b)  # 2485-2501
...
# ---- MAIN HYPOTHESIS ----
# ---- model ----
fit_cat_nmets <- with(hrv, lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+n_mets6_cat))
nmets_ci <- summary(pool(fit_cat_nmets), conf.int = TRUE)
nmets_ci
pool.r.squared(fit_cat_nmets, adjusted = TRUE) # 0.1183496
...
qplot(x=term, y=estimate, data=nmets_ci) + geom_errorbar(aes(ymin=lower, ymax=upper, width=0.15)) +
  labs(y="SDNN pre CPT", x="Number of MetS factors or diabetes")

table(hrv_na$n_mets6_cat, useNA = "ifany")
table(hrv_na$n_mets[hrv_na$n_mets6 == 6])

#write.xlsx(nmets_ci, file='plotdata_gp_pre_sdnn.xlsx', sheetName = 'nmets_ci')
...
fit.mitml <- with(implist1, lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+n_mets6_cat))
...
#write.xlsx(tuk, file='plotdata_gp_pre_sdnn.xlsx', sheetName = 'tukey', append=TRUE)




# ---- model w/ unimputed data----
f <- lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+n_mets6_cat, data=hrv_na)
summary(f)  # same as imputed (almost on the decimal)
Anova(f)
anova(f) # why different numbers?
Anova(f, lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+n_mets6_cat, data=hrv_na), type=3)
aov(f)
#est sigma 21.8


#compare variance estimate over different imputations
f <- lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+n_mets6_cat, data=complete(hrv, 2))
summary(f)  # 21.9
f <- lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+n_mets6_cat, data=complete(hrv, 15))
summary(f)  # 21.9
#same for 1, 6, 22
...
#not a good plot for categorical data
ggplot(f_mod_diag, aes(n_mets6_cat, PT_PRE_CPT_SDNN_T6)) +
  geom_point() +
  stat_smooth(method = lm, se = FALSE) +
  geom_segment(aes(xend = n_mets6_cat, yend = .fitted), color = "red", size = 0.3)
...
# ---- SPECIFIC FACTORS ----
# ---- singles ----
fit_singles <- with(hrv, lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+central_obesity+high_triglyc+
                           low_hdl+high_BP+high_glucose + diabetic))

summary(pool(fit_singles))  # hdl not significant

pool.r.squared(fit_singles, adjusted = TRUE)  # 0.1186405
...
#write.xlsx(metsfac, file='plotdata_gp_pre_sdnn.xlsx', sheetName = 'mets_factors', append=TRUE)
...
# ---- model assumptions ----
# -- no imp
noimp_singles <- lm(PT_PRE_CPT_SDNN_T6~SEX_T6+AGE_T6+central_obesity+high_triglyc+
...
# ---- pairs ----
fit_pairs <- with(hrv, lm(PT_PRE_CPT_SDNN_T6~central_obesity*high_triglyc+
...
#singles: hdl, glucose not sign
#pairs: co*BP significant

pool.r.squared(fit_pairs, adjusted = TRUE)  # 0.1186623
...
plot.new()
plot.window(xlim=c(0, 1), ylim=c(60, 80))
abline(a=74, b=-4) #CO
axis(1)
axis(2)
abline(a=68, b=-1) #BP

# BP not much affected by co, but adding high bp to co makes it worse??


# ---- unimputed ----
noimp_pairs <- lm(PT_PRE_CPT_SDNN_T6~central_obesity*high_triglyc+
             central_obesity*low_hdl + central_obesity*high_BP +
             central_obesity*high_glucose + high_triglyc*low_hdl + 
             high_triglyc*high_BP + high_triglyc*high_glucose +
             low_hdl*high_BP + low_hdl*high_glucose + high_BP*high_glucose +
...
summary(noimp_pairs)
#hdl and glucose not sign, co*bp significant
#R^2 0.1234
...
# ---- triplets ----
fit_triplets <- with(hrv, lm(PT_PRE_CPT_SDNN_T6~
...
#singles: hdl, glucose not sign
#pairs: none significant (co*bp almost)
#triplets: none significant

pool.r.squared(fit_triplets, adjusted = TRUE)  # 0.1182806
